...
let botNomi = '';
let oxirgiXato = null;
let tokenXatoAytildi = false;      // 401 xatosi logga faqat bir marta yozilsin
...
async function xabargaJavob(msg) {
  ...
  /* Mini app'dagi "Telegramdagi raqamimni yuborish" tugmasi.
     Telegram raqamni faqat shu yo'l bilan beradi — u botga xabar bo'lib keladi.
     Bunga javob yozmaymiz va chat toza qolishi uchun o'sha xabarni o'chiramiz
     (bot shaxsiy chatda kelgan xabarni o'chira oladi). */
  if (msg.contact) {
    if (!msg.contact.user_id || msg.contact.user_id === msg.from.id) {
      obunachilar.telefonSaqla(msg.from.id, msg.contact.phone_number);
    }
    try {
      await tg.api('deleteMessage', { chat_id: chat.id, message_id: msg.message_id });
    } catch (_) { /* o'chirib bo'lmasa ham muhim emas */ }
    return;
  }
...
async function sikl() {
  while (ishlayapti) {
    try {
      ...
      oxirgiXato = null;
      tokenXatoAytildi = false;
    } catch (e) {
      oxirgiXato = { vaqt: new Date().toISOString(), xabar: e.message };
      // 409 — shu token bilan boshqa joyda ham bot ishlab turibdi
      // 401 — token bekor qilingan (@BotFather). Yangi token .env ga yozilib, server qayta ishga tushirilishi kerak
      const ms = e.kod === 401 ? 60000 : e.kod === 409 ? 30000 : e.qaytaSoniya ? e.qaytaSoniya * 1000 : 5000;
      const izoh = e.kod === 401 ? ' (BOT_TOKEN yaroqsiz — @BotFather dan yangi token olib .env ga yozing)' : '';
      if (e.kod !== 401 || !tokenXatoAytildi) {
        console.error('[bot] getUpdates:', e.message + izoh, `— ${Math.round(ms / 1000)}s dan keyin qayta`);
      }
      tokenXatoAytildi = e.kod === 401;
      await kut(ms);
    }
  }
}
...
module.exports = { ishgaTushir, natijaYubor, holat, xabargaJavob, initDataTekshir: tg.initDataTekshir };
