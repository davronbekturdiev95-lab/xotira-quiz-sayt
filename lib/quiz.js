...
/** Saytda va botda ko'rinadigan (yashirilmagan) savollar */
function faolSavollar(config) {
  return (config.savollar || []).filter((s) => !s.yashirin);
}

/** Asosiy hisob */
function hisobla(config, javoblar) {
  const savollar = faolSavollar(config);   // yashirin savollar hisobga kirmaydi
...
function ommaviySavollar(config) {
  return faolSavollar(config).map((s) => ({
    id: s.id,
    matn: s.matn,
...
module.exports = { hisobla, maksimum, javoblarniTekshir, ommaviySavollar, faolSavollar, OQ_KODLAR, BELGI_KODLAR };
