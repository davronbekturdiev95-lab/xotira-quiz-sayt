...
/** t.me va tg:// havolalari — ular Telegramning o'zida, brauzersiz ochiladi */
function telegramHavolasimi(url) {
  return /^(https?:\/\/(t\.me|telegram\.me|telegram\.dog)\/|tg:\/\/)/i.test(String(url || ''));
}

/**
 * Tugma havolasi.
 * Telegram havolalari to'g'ridan-to'g'ri qo'yiladi — agar ularni bosilishini
 * sanash uchun /r/<kod> orqali yo'naltirsak, odam avval brauzerga chiqib,
 * keyin Telegramga qaytadi (chunki bu havolalar boshqa bot/mini app'ga ketadi).
 * Oddiy sayt havolalari esa baribir brauzerda ochiladi — ularni sanayveramiz.
 */
function kuzatiladiganUrl(tgId, kontekst, url) {
  const manzil = String(url || '').replace(/^video:/, '');
  if (!SAYT || telegramHavolasimi(manzil)) return manzil;
  return SAYT + 'r/' + kodSaqla(tgId, kontekst, url);
}
...
module.exports = {
  ...
  kodSaqla, kuzatiladiganUrl, kodBosildi, telegramHavolasimi,
  yubor, yuborHtml
};
