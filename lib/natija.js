...
/** Muammo foizi qaysi darajaga tushishini topadi (chegaralar admin paneldan) */
function darajaTop(darajalar, muammoFoiz) {
  const tartib = (darajalar || []).slice().sort((a, b) => num(a.max) - num(b.max));
  let d = tartib[tartib.length - 1] || { nom: '', kod: '', matn: '' };
  for (const x of tartib) if (muammoFoiz <= num(x.max)) { d = x; break; }
  return {
    kod: d.kod || '', nom: d.nom || '', matn: d.matn || '',
    tartib: Math.max(0, tartib.indexOf(d)), jami: tartib.length
  };
}
...
function tahlil(config, javoblar, h, opts = {}) {
  ...
  /* ---- Halqalar: xotira kuchi va diqqat ----
     Xotira kuchi — unutuvchanlik savollaridan (100 - daraja foizi).
     Diqqat — diqqat o'qidagi muammo foizidan (100 - muammo). Darajasi ikkalasida bir xil
     chegaralar bo'yicha aniqlanadi. Diqqatga ball beradigan savol qolmasa — halqa chiqmaydi. */
  const xotiraKuchi = Math.max(0, Math.min(100, 100 - num(h.darajaFoiz)));
  const halqalar = [{
    kod: 'xotira',
    nom: M.natija_kuch_nom || 'Xotira kuchi',
    kuch: xotiraKuchi,
    daraja: {
      kod: h.darajaKod || '', nom: h.daraja || '', matn: h.darajaMatn || '',
      tartib: darajaIndeks, jami: darajalar.length
    }
  }];

  const maksimum = quiz.maksimum(savollar);
  if (maksimum.diqqat > 0) {
    const muammo = Math.max(0, Math.min(100, Math.round(num(h.foiz && h.foiz.diqqat))));
    halqalar.push({
      kod: 'diqqat',
      nom: M.natija_diqqat_nom || 'Diqqatingiz',
      kuch: 100 - muammo,
      daraja: darajaTop(darajalar, muammo)
    });
  }
...
function telegramMatn(t, M = {}) {
  ...
  const halqalar = t.halqalar && t.halqalar.length
    ? t.halqalar
  ...
  for (const g of halqalar) {
    asosiy.push(`${esc(g.nom)}: <b>${g.kuch}/100</b>${g.daraja && g.daraja.nom ? ' — ' + esc(g.daraja.nom) : ''}`);
    asosiy.push(shkala(g.kuch));
  }
  asosiy.push(
    '',
