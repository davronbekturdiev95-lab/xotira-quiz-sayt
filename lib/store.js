...
const { boshlangichConfig, VIDEO_FOYDALAR } = require('../shared/defaults.js');
...
function tolatir(cfg) {
  ...
  // Natija izohlari (muammo / kuchli / reja) va video foydalari keyinroq qo'shildi.
  // Faqat bir marta, matni o'zgartirilmagan standart savol va videolarga qo'yiladi.
  if (!cfg.izohVersiya) {
    const defS = indeksla(def.savollar, 'id');
    for (const s of cfg.savollar) {
      const ds = defS[s.id];
      if (!ds) continue;
      for (const v of s.variantlar || []) {
        const dv = ds.variantlar.find((x) => x.key === v.key && x.matn === v.matn);
        if (!dv) continue;
        for (const k of ['muammo', 'kuchli', 'reja']) if (v[k] === undefined && dv[k]) v[k] = dv[k];
      }
    }
    for (const v of cfg.videolar) {
      if (v.foydalar === undefined && VIDEO_FOYDALAR[v.id]) v.foydalar = VIDEO_FOYDALAR[v.id];
    }
    cfg.izohVersiya = 1;
  }
...
function farqla(eski, yangi) {
  ...
  for (const id of Object.keys(yangiS)) {
    ...
    if (!!a.yashirin !== !!b.yashirin) {
      o.push(`${raqam}-savol ${b.yashirin ? 'yashirildi (saytda va botda ko\'rinmaydi)' : 'qayta ko\'rsatildi'}: "${qisqa(b.matn)}"`);
    }
    ...
    for (const k of Object.keys(bV)) {
      ...
      const izohNom = { muammo: 'muammo izohi', kuchli: 'kuchli tomon izohi', reja: 'kunlik reja matni' };
      for (const iz of Object.keys(izohNom)) {
        if ((aV[k][iz] || '') !== (bV[k][iz] || '')) {
          o.push(`${raqam}-savol, ${k} varianti ${izohNom[iz]}: "${qisqa(aV[k][iz] || '—')}" → "${qisqa(bV[k][iz] || '—')}"`);
        }
      }
    }
  ...
  for (const id of Object.keys(yangiV)) {
    ...
    if ((a.poster || null) !== (b.poster || null)) {
      o.push(`${id.toUpperCase()} posteri: ${!b.poster ? 'olib tashlandi' : !a.poster ? 'rasm qo\'yildi' : 'boshqa rasmga almashtirildi'}`);
    }
    if ((a.foydalar || '') !== (b.foydalar || '')) o.push(`${id.toUpperCase()} "videoda nimalarni o'rganasiz" ro'yxati o'zgartirildi`);
  }
